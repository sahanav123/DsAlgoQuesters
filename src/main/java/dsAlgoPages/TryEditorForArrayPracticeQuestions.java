...
public class TryEditorForArrayPracticeQuestions {
	...
	public TryEditorForArrayPracticeQuestions(WebDriver driver) {
		this.driver = driver;
	...
	public void enterCodePractice(String code, WebElement element) {
		// Clear the previous code in the editor (Ctrl + A, Delete)
		new Actions(driver).keyDown(Keys.COMMAND).keyUp(Keys.COMMAND).perform();

		// Split the code by line breaks
		String[] str1 = code.split("\n");

		// Iterate over each line of code and send it to the input element
		for (int i = 0; i < str1.length; i++) {
			if (str1[i].equalsIgnoreCase("\\b")) {
				// Handle backspace
				element.sendKeys(Keys.BACK_SPACE);
			} else {
				// Enter the line of code and simulate pressing "Enter" (Return)
